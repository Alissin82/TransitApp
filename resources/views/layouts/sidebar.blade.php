...
<aside id="sidebar"
        ...
        initializeActiveMenus() {
            @foreach ($menuGroups as $groupIndex => $menuGroup)
                @foreach ($menuGroup['items'] as $itemIndex => $item)
                    @if (isset($item['subItems']) && MenuHelper::hasActiveSubItem($item))
    ...
    <div class="pb-7 pt-8 flex"
         :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ? 'xl:justify-center' : 'justify-start'">
        <a href="{{ route('home') }}">
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                 class="dark:hidden" src="{{ asset('images/logo/logo.svg') }}" alt="{{ __('Logo') }}" width="150" height="40" />
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                 class="hidden dark:block" src="{{ asset('images/logo/logo-dark.svg') }}" alt="{{ __('Logo') }}" width="150" height="40" />
            <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                 src="{{ asset('images/logo/logo-icon.svg') }}" alt="{{ __('Logo') }}" width="32" height="32" />
        </a>
    </div>
                                ...
                                <li>
                                    @if (isset($item['subItems']))
                                        <button
                                                ...
                                                :class="[
                                                (isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) || {{ MenuHelper::hasActiveSubItem($item) ? 'true' : 'false' }})
                                                    ? 'menu-item-active'
                                                    : 'menu-item-inactive',
                                            ...
                                            <span
                                                    :class="(isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) || {{ MenuHelper::hasActiveSubItem($item) ? 'true' : 'false' }})
                                            ...
                                            <svg
                                                    ...
                                                    :class="{
                                                    'rotate-180 text-brand-500': isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) || {{ MenuHelper::hasActiveSubItem($item) ? 'true' : 'false' }}
                                                }"
                                    ...
                                    @else
                                        <a
                                                ...
                                                :class="[
                                                '{{ MenuHelper::isActive($item) ? 'menu-item-active' : 'menu-item-inactive' }}',
                                                (!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen)
                                                    ? 'xl:justify-center'
                                                    : 'justify-start'
                                            ]">

                                            <span class="{{ MenuHelper::isActive($item) ? 'menu-item-icon-active' : 'menu-item-icon-inactive' }}">
                                                {!! MenuHelper::getIcon($item['icon']) !!}
                                            </span>
